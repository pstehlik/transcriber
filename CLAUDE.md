...
- **stdout line parsing**: Each segment from `--verbose True` looks like `[00:00.000 --> 00:09.360] transcribed text`. `parseSegmentLine()` in `transcriber.js` captures start, end, and text; lines that don't match are treated as log output.
- **Repetition-loop filtering**: Whisper can fall into a repetition loop near the end of a file, appending dozens of copies of the last phrase (measured: 79x "Ciao." on a 94s voice message; 8x "Schützende Kleidung." on a 63s file, running out to 01:21 — 18s past the audio's end). `createSegmentFilter(durationSeconds)` in `transcriber.js` returns a stateful predicate rejecting three shapes: (1) zero-duration segments, where `start === end` and no audio is spanned; (2) segments starting at or after the end of the audio — Whisper zero-pads the final 30s window and decodes into the padding; (3) consecutive segments repeating the previous text (`MAX_CONSECUTIVE_IDENTICAL`, currently 1 — raise it to allow genuine doubling through). Dropped segments are counted and logged once at completion. **Turbo loops far more readily than the deeper models** because its decoder is pruned to 4 layers, which is why this surfaced in 1.3.0.
- **Repetition loops are not fixable via decoding flags** (measured, do not retry): `--condition-on-previous-text False` stops the loop but fragments segmentation and introduces word errors ("der Herbstmarathon" → "unser Hauptmarathon"); `--word-timestamps True` also stops it but likewise changes transcribed words; `--compression-ratio-threshold` does nothing at all, because the CLI passes a scalar `temperature: 0` and the ratio check exists only to escalate to a fallback temperature tuple that is never constructed. Filtering the output is strictly cheaper than degrading the decode.
- **Audio duration needs `{ duration: true }`**: `music-metadata` returns `format.duration === undefined` for Ogg/Opus — i.e. every WhatsApp voice note — because the header carries no duration. `getFileMetadata()` passes `{ duration: true }` so the file is scanned (~10ms). Without it, the past-the-end rule above is silently inert on exactly the app's most common input. Duration flows metadata → `startTranscriptionRun()` → `startTranscription(..., durationSeconds)`; it is `null` when unknown and the rule is then skipped rather than guessed.
